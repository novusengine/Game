...
namespace
{
    ...
    const Material::ParameterDefinition* FindParameter(std::span<const Material::ParameterDefinition> parameters, u32 byteOffset)
    {
        const auto it = std::find_if(parameters.begin(), parameters.end(), [byteOffset](const Material::ParameterDefinition& parameter) {
            return byteOffset >= parameter.byteOffset && static_cast<u64>(byteOffset) + sizeof(u32) <= static_cast<u64>(parameter.byteOffset) + parameter.byteSize;
        });
...
namespace MaterialLoading
{
    MaterialAssetReadResult<MaterialAssetView> MaterialAssetValidator::ValidateMaterial(MaterialAssetReadResult<MaterialAssetView> result)
    {
        ...
        if (root.programKey == Material::INVALID_MATERIAL_PROGRAM_KEY)
            return Fail<MaterialAssetView>(DiagnosticCode::InvalidValue, "MaterialAsset.programKey");
        if (root.reserved0 != 0)
        ...
        if (!IsPowerOfTwo(root.parameterBlockAlignment) || root.parameterBlockAlignment < alignof(u32) || root.parameterBlockSize % root.parameterBlockAlignment != 0)
            return Fail<MaterialAssetView>(DiagnosticCode::InvalidValue, "MaterialAsset.parameterBlockAlignment", Diagnostic::NO_INDEX, root.parameterBlockAlignment, root.parameterBlockSize);
        if (root.parameterBlockSize != root.defaultParameterDataSize)
            return Fail<MaterialAssetView>(DiagnosticCode::CountMismatch, "MaterialAsset.defaultParameterDataSize", Diagnostic::NO_INDEX, root.defaultParameterDataSize, root.parameterBlockSize);
        ...
        for (u32 index = 0; index < result.view.parameters.size(); ++index)
        {
            ...
            if (parameter.type > Material::ParameterType::Sampler)
                return Fail<MaterialAssetView>(DiagnosticCode::UnsupportedEnum, "parameters.type", index, static_cast<u8>(parameter.type), static_cast<u8>(Material::ParameterType::Sampler));
            const u32 elementSize = ParameterElementSize(parameter.type);
            if (parameter.nameHash == 0 || parameter.arrayCount == 0 || parameter.byteSize != elementSize * parameter.arrayCount || parameter.byteOffset % alignof(u32) != 0 ||
                parameter.byteOffset > root.parameterBlockSize || parameter.byteSize > root.parameterBlockSize - parameter.byteOffset)
                return Fail<MaterialAssetView>(DiagnosticCode::InvalidValue, "parameters.layout", index, parameter.byteSize, elementSize * parameter.arrayCount);
    ...
    MaterialAssetReadResult<MaterialInstanceAssetView> MaterialAssetValidator::ValidateMaterialInstance(MaterialAssetReadResult<MaterialInstanceAssetView> result, const MaterialAssetView& material)
    {
        ...
        if (root.parameterDataSize != material.root.parameterBlockSize)
            return Fail<MaterialInstanceAssetView>(DiagnosticCode::CountMismatch, "MaterialInstanceAsset.parameterDataSize", Diagnostic::NO_INDEX, root.parameterDataSize, material.root.parameterBlockSize);
        ...
        if (root.parameterLayoutHash != parameterLayoutHash)
            return Fail<MaterialInstanceAssetView>(DiagnosticCode::LayoutHashMismatch, "MaterialInstanceAsset.parameterLayoutHash", Diagnostic::NO_INDEX, root.parameterLayoutHash, parameterLayoutHash);

        if (root.rasterClass > Material::RasterClass::Transparent)
            return Fail<MaterialInstanceAssetView>(DiagnosticCode::UnsupportedEnum, "MaterialInstanceAsset.rasterClass", Diagnostic::NO_INDEX, static_cast<u8>(root.rasterClass),
                                                   static_cast<u8>(Material::RasterClass::Transparent));
        if ((root.flags & ~MATERIAL_INSTANCE_FLAGS) != 0)
            return Fail<MaterialInstanceAssetView>(DiagnosticCode::UnsupportedFlags, "MaterialInstanceAsset.flags", Diagnostic::NO_INDEX, root.flags, MATERIAL_INSTANCE_FLAGS);
        if (root.reserved0 != 0)
            return Fail<MaterialInstanceAssetView>(DiagnosticCode::ReservedValueNonZero, "MaterialInstanceAsset.reserved0");

        std::unordered_set<u32> textureSlots;
        for (u32 index = 0; index < result.view.textureBindings.size(); ++index)
        {
            const Material::TextureBinding& binding = result.view.textureBindings[index];
            if (binding.type > Material::ResourceType::Sampler)
                return Fail<MaterialInstanceAssetView>(DiagnosticCode::UnsupportedEnum, "textureBindings.type", index, static_cast<u8>(binding.type), static_cast<u8>(Material::ResourceType::Sampler));
            if ((binding.flags & ~RESOURCE_FLAGS) != 0)
                return Fail<MaterialInstanceAssetView>(DiagnosticCode::UnsupportedFlags, "textureBindings.flags", index, binding.flags, RESOURCE_FLAGS);
            if (binding.textureSlot >= material.root.textureSlotCount || !textureSlots.insert(binding.textureSlot).second)
                return Fail<MaterialInstanceAssetView>(DiagnosticCode::InvalidRange, "textureBindings.textureSlot", index, binding.textureSlot, material.root.textureSlotCount);
            if (binding.resourceAssetID == FileFormat::INVALID_ASSET_ID)
            {
                if ((binding.flags & Material::ResourceBindingFlags_Optional) == 0)
                    return Fail<MaterialInstanceAssetView>(DiagnosticCode::MissingRequiredReference, "textureBindings.resourceAssetID", index);
        ...
        for (u32 index = 0; index < result.view.animationBindings.size(); ++index)
        {
            ...
            if (binding.timeSource > Material::AnimationTimeSource::AnimationController)
                return Fail<MaterialInstanceAssetView>(DiagnosticCode::UnsupportedEnum, "animationBindings.timeSource", index, static_cast<u8>(binding.timeSource),
            ...
            if (binding.parameterByteOffset % alignof(u32) != 0 || binding.parameterByteOffset > root.parameterDataSize || sizeof(u32) > root.parameterDataSize - binding.parameterByteOffset ||
                !FindParameter(material.parameters, binding.parameterByteOffset))
                return Fail<MaterialInstanceAssetView>(DiagnosticCode::InvalidRange, "animationBindings.parameterByteOffset", index, binding.parameterByteOffset, root.parameterDataSize);
        }

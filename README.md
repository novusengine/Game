...
## How to use
1. You must take the `Data` folder generated by the `AssetConverter.exe` (see [AssetConverter](https://github.com/novusengine/AssetConverter)) and place it in the `Game/Build/` folder.
>[!TIP]
>It is highly recommended to [symlink](https://schinagl.priv.at/nt/hardlinkshellext/linkshellextension.html) the `Data` folder to avoid copying/mobing a large (~16GB) folder.
